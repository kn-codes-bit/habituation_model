...
int compute_ht(double thres, double T_on, double T_off, int m, double *p0, double *para, double *para2);
double compute_rt(double thres, double T_on, double T_off, int m, double *p0, double *para, double *para2);

int main(int argc, const char* argv[])
{
    double T_on = 8., T_off, rt;
    ...
    */
    ...
    {   
        ...
        printf("T_s=%f, T=%f\n", T_on, T_on+T_off);
        habi_th = 0.01;
        //ht = compute_ht(habi_th,T_on, T_off, m, p0, para, para2);
        rt = compute_rt(habi_th, T_on, T_off, m, p0, para, para2);
        printf("rt=%f (usual)\n", rt);
        habi_th = 0.005;
        //ht = compute_ht(habi_th,T_on, T_off, m, p0, para, para2);
        rt = compute_rt(habi_th, T_on, T_off, m, p0, para, para2);
        printf("rt=%f (more stimulation)\n\n", rt);
    }
...
double compute_rt(double thres, double T_on, double T_off, int m, double *p0, double *para, double *para2)
{
    ...
    ht = compute_ht(thres, T_on, T_off, m, x0, para, para2);
    for (int i = 0; i < ht; i++)
    {
        sol_1(0., T_on, x0, x, para, para2);
        sol_0(0., T_off, x, x0, para, para2);
